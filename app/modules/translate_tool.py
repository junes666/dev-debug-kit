        ...
        with self._lock:
            ...
            try:
                self._proc.stdin.write(json.dumps(
                    {"text": text, "source": source, "target": target}, ensure_ascii=False) + "\n")
            ...
            if not resp.get("ok"):
                raise RuntimeError(resp.get("error", "翻译失败"))
...
class TranslateTool(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.btn_download = widgets.primary("下载翻译组件", self._on_download,
                                            "下载翻译运行库与中英模型（约180MB，一次性，之后离线）")
        self.btn_download.hide()

        root.addWidget(widgets.row(
            widgets.label("方向", "label"), self.dir_combo, None,
            self.btn_download, self.btn_paste, self.btn_translate, self.btn_clear, self.btn_copy,
        ))
